...
int ComprDataIO::UnpRead(byte *Addr,size_t Count)
{
  ...
  while (Count > 0)
  {
    ...
    if (UnpackFromMemory)
    {
      if (UnpackFromMemorySize==0)
        break; // EOF from memory buffer — exit while loop
      size_t CopySize=Count<UnpackFromMemorySize ? Count : UnpackFromMemorySize;
      memcpy(Addr,UnpackFromMemoryAddr,CopySize);
      ReadSize=(int)CopySize;
      UnpackFromMemoryAddr+=CopySize;
      UnpackFromMemorySize-=CopySize;
    }
    else
    {
